...
<PageHeader
  ...
  breadcrumb={[
    { label: 'Opportunities', href: '/opportunities' },
    {
      ...
      href: opp?.id ? `/opportunities/${opp.id}` : '/opportunities'
    },
    { label: 'Edit' }
  ]}
/>
...
<FormShell errorMessage={form?.error || ''} useEnhance={handleSubmit}>
    ...
    <div class="flex flex-col gap-1.5">
      ...
      >
        {#each stageChoices as opt (opt.value)}
          <option value={opt.value} selected={opt.value === fv('stage', opp?.stage)}
            >{opt.label}</option
          >
        {/each}
      </select>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        >
          {#each currencyChoices as c (c.value)}
            <option value={c.value} selected={c.value === fv('currency', opp?.currency || 'USD')}
              >{c.value}</option
            >
          {/each}
        </select>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1.5">
        ...
        <Input
          ...
          max="100"
          step="1"
          value={fv('probability', opp?.probability)}
          placeholder="0–100"
        />
        <span class="text-[11px] text-[color:var(--text-subtle)]"
          >Percent likelihood of closing.</span
        >
      </div>
      <div class="flex flex-col gap-1.5">
        ...
        <Input
          ...
          name="closeDate"
          type="date"
          value={fv('closeDate', opp?.close_date)}
        />
      </div>
    </div>
  </section>
  ...
  <section class="flex flex-col gap-3">
    ...
    <div class="flex flex-col gap-1.5">
      ...
      >
        ...
        {#each sourceChoices as opt (opt.value)}
          <option value={opt.value} selected={opt.value === fv('leadSource', opp?.lead_source)}
            >{opt.label}</option
          >
        {/each}
      </select>
    ...
    <div class="flex flex-col gap-1.5">
      ...
      <textarea
        ...
        >{fv('description', opp?.description)}</textarea
      >
    </div>
  </section>
  ...
  {#snippet actions()}
    <Button
      type="button"
      variant="ghost"
      onclick={() => goto(opp?.id ? `/opportunities/${opp.id}` : '/opportunities')}
    >
      Cancel
    </Button>

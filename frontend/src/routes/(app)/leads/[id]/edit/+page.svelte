...
<FormShell errorMessage={form?.error || ''} useEnhance={handleSubmit}>
  ...
  <section class="flex flex-col gap-3">
    <h3 class="text-[11px] font-semibold tracking-wider text-[color:var(--text-subtle)] uppercase">
      Basics
    </h3>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        >
          {#each statusChoices as opt (opt.value)}
            <option
              value={opt.value}
              selected={opt.value === normalizeStatus(fv('status', lead?.status))}
              >{opt.label}</option
            >
          {/each}
        </select>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        >
          ...
          {#each sourceChoices as opt (opt.value)}
            <option value={opt.value} selected={opt.value === fv('source', lead?.source)}
              >{opt.label}</option
            >
          {/each}
        </select>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        >
          ...
          {#each ratingChoices as opt (opt.value)}
            <option
              value={opt.value}
              selected={opt.value === normalizeRating(fv('rating', lead?.rating))}
              >{opt.label}</option
            >
          {/each}
        </select>
    ...
    <div class="grid grid-cols-[1fr_140px] gap-3">
      <div class="flex flex-col gap-1.5">
        ...
        <Input
          ...
          min="0"
          step="0.01"
          value={fv('opportunityAmount', lead?.opportunity_amount)}
          placeholder="0.00"
        />
      </div>
      <div class="flex flex-col gap-1.5">
        ...
        >
          {#each currencyChoices as c (c.value)}
            <option value={c.value} selected={c.value === fv('currency', lead?.currency || 'USD')}
              >{c.value}</option
            >
          {/each}
        </select>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1.5">
        ...
        <Input
          ...
          max="100"
          step="1"
          value={fv('probability', lead?.probability)}
          placeholder="0–100"
        />
      </div>
      <div class="flex flex-col gap-1.5">
  ...
  <section class="flex flex-col gap-3">
    ...
    <div class="grid grid-cols-[1fr_120px_140px] gap-3">
      <div class="flex flex-col gap-1.5">
        ...
        <Input id="lead-city" name="city" maxlength="100" value={fv('city', lead?.city)} />
      </div>
      <div class="flex flex-col gap-1.5">
        <Label for="lead-state">State</Label>
        <Input id="lead-state" name="state" maxlength="100" value={fv('state', lead?.state)} />
      </div>
      <div class="flex flex-col gap-1.5">
  ...
  <section class="flex flex-col gap-3">
    ...
    <div class="flex flex-col gap-1.5">
      ...
      <textarea
        ...
        name="description"
        rows="5"
        class="flex w-full resize-y rounded-[var(--r-md)] border border-[color:var(--border)] bg-[color:var(--bg-input)] px-3 py-2 text-[13px] text-[color:var(--text)] outline-none hover:border-[color:var(--border-strong)] focus-visible:border-[color:var(--text)] focus-visible:shadow-[0_0_0_3px_var(--focus-ring)]"
        >{fv('description', lead?.description)}</textarea
      >
    </div>
  </section>
  ...
  {#snippet actions()}
    <Button
      type="button"
      variant="ghost"
      onclick={() => goto(lead?.id ? `/leads/${lead.id}` : '/leads')}
    >
      Cancel
    </Button>

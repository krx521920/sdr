<script>
  ...
  let users = $derived(
    Array.isArray(data.users)
      ? data.users.map((u) => ({
          ...
          email: u.user.email,
          role: u.role,
          matchingAccessLevel: ['none', 'read', 'manage', 'recompute', 'decide'].includes(
            u.matchingAccessLevel
          )
            ? u.matchingAccessLevel
            : 'none',
  ...
  /** @type {string | null} */
  let editingMatchingAccessId = $state(null);
  ...
  $effect(() => {
    if (form?.success) {
      ...
      } else if (form.action === 'edit_matching_access') {
        toast.success('Matching access updated');
        editingMatchingAccessId = null;
      }
      invalidateAll();
    } else if (form?.error) {
  ...
  /** @param {string} value */
  function matchingAccessLabel(value) {
    return value === 'none' ? 'None' : value.charAt(0).toUpperCase() + value.slice(1).toLowerCase();
  }
              ...
              <Table.Root>
                <Table.Header>
                  <Table.Row>
                    ...
                    <Table.Head>Matching access</Table.Head>
                ...
                <Table.Body>
                  {#each users as user (user.id)}
                    <Table.Row>
                      ...
                      <Table.Cell>
                        {#if user.isSelf || editingMatchingAccessId !== user.id}
                          <Badge
                            variant={user.matchingAccessLevel === 'none' ? 'outline' : 'secondary'}
                            class="cursor-default"
                          >
                            {matchingAccessLabel(user.matchingAccessLevel)}
                          </Badge>
                          {#if !user.isSelf}
                            <Button
                              variant="ghost"
                              ...
                              aria-label="Edit matching access for {user.name}"
                              onclick={() => (editingMatchingAccessId = user.id)}
                            >
                              <Edit class="h-3 w-3" />
                            </Button>
                          {/if}
                        {:else}
                          <form
                            method="POST"
                            action="?/edit_matching_access"
                            class="flex items-center gap-2"
                          >
                            <input type="hidden" name="user_id" value={user.id} />
                            <label class="sr-only" for="matching-access-{user.id}"
                              >Matching access for {user.name}</label
                            >
                            <select
                              id="matching-access-{user.id}"
                              name="matching_access_level"
                              class="border-input bg-background h-8 rounded-md border px-2 text-sm"
                            >
                              <option value="none" selected={user.matchingAccessLevel === 'none'}
                                >None</option
                              >
                              <option value="read" selected={user.matchingAccessLevel === 'read'}
                                >Read</option
                              >
                              <option
                                value="manage"
                                selected={user.matchingAccessLevel === 'manage'}>Manage</option
                              >
                              <option
                                value="recompute"
                                selected={user.matchingAccessLevel === 'recompute'}
                                >Recompute</option
                              >
                              <option
                                value="decide"
                                selected={user.matchingAccessLevel === 'decide'}>Decide</option
                              >
                            </select>
                            <Button
                              type="submit"
                              ...
                              class="h-7 w-7"
                              variant="default"
                              aria-label="Save matching access for {user.name}"
                            >
                            ...
                            <Button
                              ...
                              class="h-7 w-7"
                              variant="outline"
                              aria-label="Cancel matching access edit for {user.name}"
                              onclick={() => (editingMatchingAccessId = null)}
                            >
                              <X class="h-3.5 w-3.5" />
                            </Button>
                          </form>
